...
# css for sidebar_layout
container_style_css <- "gap: 0;"
sidebar_panel_css <- "padding: 1em;
                      background-color: #eff0f1;
                      border: 0.5px solid #561643;
                      border-right: 0;
                      border-top-left-radius: 7px;
                      border-top-right-radius: 0px;
                      border-bottom-left-radius: 7px;
                      border-bottom-right-radius: 0px;"
main_panel_css <- "padding: 1em; 
...
ui <- semanticPage(
  ...
  titlePanel(div("Tidy Tuesday: Project FeederWatch", id = "title_panel")),
  
  tabset(menu_class = "ui secondary pointing menu",
    tabs = list(
      list(menu = "Project FeederWatch",
           content = div(class = "ui basic segment",
                         sidebar_layout(sidebar_panel(width = 4, # sidebar layout creates a grid that contains two panels. Each element created is given a randomly generated id
                                                      div(h3("Select a species"),
                                            ...
                                            main_panel(width = 12,
                                                       div(h2("Map"),
                                                           br(),
                                                           p("With this dashboard, you can quickly visualise where species of bird were observed in North America by the FeederWatch community over Winter 2020/2021. Select a species of interest in the menu on the left and the map below will update."),
                                                           br(),
                                                           textOutput("graph_title"),
                                                           br(),
                                                           card(plotOutput("species_location"), class = "ui centered class"),
                                                           br(),
                                                           h3("Plotted Data"),
                                                           p("The table below describes the data shown in the map"),
                                                           semantic_DTOutput("species_table")
                                                           )
                                                       ),
                                        container_style = container_style_css, 
                                        area_styles = list(sidebar_panel = sidebar_panel_css, main_panel = main_panel_css)
                                        )
                         ),
           id = "Project_FeederWatch_tab"
          ),
      
      list(menu = "About",
           content = div(class = "ui basic segment",
                         div(h3("What is Project FeederWatch?")),
                                 ...
                                 br(),
                         div(h3("Why are these data important?")),
