...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "Tidy Tuesday: Project FeederWatch",
                  titleWidth = 330),
  
  dashboardSidebar(
                   sidebarMenu(
                               menuItem("Project FeederWatch",
                                        tabName = "Project_FeederWatch",
                                        icon = icon("feather")
                                        ),
                               menuItem("About",
                                        tabName = "About",
  ...
  dashboardBody(
                tabItems(
                         tabItem(tabName = "Project_FeederWatch",
                                 fluidRow(
                                          column(6,
                                                 h2("Map"),
                                                 p("With this dashboard, you can quickly visualise where species of bird were observed in North America by the FeederWatch community over Winter 2020/2021. Select a species of interest in the dropdown list and the map below will update."),
                                          ...
                                          column(6,
                                                 valueBoxOutput("species_value_box", width = NULL))
                                 ),
                                 br(),
                                 
                                 fluidRow(box(textOutput("graph_title"),
                                              plotOutput("species_location"), width = 12)),
                                 br(),
                                 
                                 fluidRow(box(p("The table below describes the data shown in the map"),
...
server <- function(input, output) {
  ...
  value_box_subtitle <- eventReactive(input$button, {
    paste0("Sightings of ", input$selected_species)
  }, ignoreNULL = F)
  
  output$species_value_box <- renderValueBox({
    valueBox(value = total_sightings(), subtitle = value_box_subtitle(), icon = icon("binoculars"), color = "olive")
  })
